...
public class StateManager {
    ...
    private void restoreFromState(HotelState state) {
        ...
        // Восстанавливаем guest -> services
        ...
        // Восстанавливаем room -> guests
        for (Map.Entry<Long, List<Long>> entry : state.roomToGuests.entrySet()) {
            Room room = roomMap.get(entry.getKey());
            if (room != null) {
                List<Guest> guests = entry.getValue().stream()
                        .map(guestMap::get)
                        .filter(Objects::nonNull)
                        .collect(Collectors.toList());
